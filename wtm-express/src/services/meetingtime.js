...
const dateToString = (date) => {
    const hour = (Math.floor(date.time / 60)).toString().padStart(2, '0');
...
const findFreeIntervals = (overlappingIntervals, time_start, time_end, days) => {
    ...
    for (const dayInterval of dayIntervals) {
        ...
        for (const busy of overlappingIntervals) {
            ...
            if (compareDates(busy.end, dayInterval.start) <= 0 || compareDates(busy.start, dayInterval.end) >= 0) {
            ...
            currentStart = compareDates(busy.end, currentStart) > 0 ? busy.end : currentStart;
        }
...
export const findMeetingTimes = async (userIds, days, time_start, time_end) => {
    ...
    try {
        ...
        const freeIntervals = findFreeIntervals(overlappingEvents, time_start, time_end, days);
